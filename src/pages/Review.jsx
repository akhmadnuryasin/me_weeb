    ...
    return (
        <Layout>
            {loading ? (
                ...
                <div>
                    <h1 className='px-5 my-8 text-3xl font-bold text-center text-white'>Anime & Manga Reviews</h1>
                    <div className="grid grid-cols-1 gap-4 px-4 pt-16 pb-8 text-white bg-black reviews-container sm:grid-cols-2 md:grid-cols-3">
                        {reviews.map(review => (
                            <div key={review.mal_id} className="flex items-start p-4 bg-black rounded-lg review-card">
                                <img src={review.entry.images.jpg.large_image_url} alt={review.entry.title} className="flex-shrink-0 w-32 h-32 rounded-lg shadow-md" />
                                <div className="flex-1 ml-4">
                                    <div className="mb-4">
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-lg font-semibold">{review.entry.title}</h3>
                                        </div>
                                        ...
                                        <div className="flex">
                                            ...
                                            {[...Array(scoreToStars(review.score).emptyStars)].map((_, index) => (
                                                <svg key={index} className="w-6 h-6 text-gray-400 fill-current" viewBox="0 0 24 24">
                                                    <path d="M12 2.6l1.8 5.9h6.1l-4.9 3.6 1.8 5.9-4.9-3.6-4.9 3.6 1.8-5.9-4.9-3.6h6.1z" />
                                                </svg>
                                            ))}
                                        </div>
